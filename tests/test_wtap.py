...
def test_damage_assessment_calculation():
    ...
    assert 9.915 == wtap_damage_assessment(
        probabilities=g, assignment=assignment, target_values=target_values
    )
...
def test_basic_wtap():
    ...
    value, assignments = wtap_solver(
        probabilities=g, weapons=weapons, target_values=target_values
    )
...
def test_exhaust_all_initialisation_permutations():
    ...
    while perms:
        ...
        perms.remove(perm2)
        damage1, ass1 = wtap_solver(
            probabilities=g, weapons=list(perm), target_values=target_values
        )
        damage2, ass2 = wtap_solver(
            probabilities=g, weapons=list(perm2), target_values=target_values
        )
...
def test_exhaustive_search_to_verify_wtap():
    ...
    for perm in permutations(weapons, len(weapons)):
        for combination in combinations_with_replacement([1, 2, 3], len(weapons)):
            ...
            r = wtap_damage_assessment(
                probabilities=g, assignment=a, target_values=target_values
            )
            if r < best_result:
                best_result = r
            ...
            c += 1
    print(
        "{} is best result out of {:,} options(exhaustive search):\n{}".format(
            best_result, c, best_assignment
        )
    )
