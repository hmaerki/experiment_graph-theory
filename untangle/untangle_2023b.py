...
def create_graph(str_game: str) -> Graph:
    ...
    for node_1, node_2 in list_edges:
        ...
        def get_node(node_name: str) -> NodeAttr:
            try:
                node_attrib = dict_nodes[node_name]
            except KeyError:
                node_attrib = NodeAttr(name=node_name)
...
class PathCollector:
    def __init__(self):
        self.path_set: Set[FrozenSet[NodeAttr]] = set()

    def iter_path(self, g: Graph) -> Set[NodeAttr]:
        """
        ...
        def recurse(path_len: int, start_node: NodeAttr, node: NodeAttr):
            ...
            for edge in sorted(g.edges(from_node=node)):
                ...
                if next_node is start_node:
                    ...
                    # else:
                    #     print("Already in set", text_path(current_path))
                    continue
...
def text_path(path: Set[NodeAttr]) -> str:
