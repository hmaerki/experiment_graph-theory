...
from graph.traffic_scheduling_problem import jam_solver, UnSolvable, NoSolution
...
def test_hill_climb_with_edge_weights():
    ...
    """
    g = Graph()
    for sed in [(1, 2, 1), (2, 3, 1), (1, 4, 3), (4, 3, 1)]:  # <-- 3!
...
def test_hill_climb_with_edge_different_weights():
    ...
    """
    g = Graph()
    for sed in [(1, 2, 3), (2, 3, 1), (1, 4, 1), (4, 3, 1)]:  # <-- 3!
        g.add_edge(*sed, bidirectional=True)
...
def test_hill_climb_with_restrictions():
    ...
    loads = {1: (1, [3], [2]), 2: [3, 1]}  # restriction 1 cannot travel over 2
...
def test_energy_and_restrictions_2_loads():
    ...
    for sed in [
        ...
        (3, 4, 1),
        (4, 6, 1),  # this could be the shortest path for load 1, but
        ...
        (
            ...
            6,
            3,
        ),  # this is the shortest unrestricted route for load 1: [2,3,6] and it is 4 long.
        (2, 6, 8),  # this is the most direct route for load 1 [2,6] but it is 8 long.
    ]:
        g.add_edge(*sed, bidirectional=True)

    loads = {1: (2, [6], [4]), 2: (3, 1)}  # restriction load 1 cannot travel over 4
...
def test_energy_and_restrictions_3_loads_b():
    ...
    loads = {
        ...
        2: (3, 1),
        3: (4, 3),
    }  # Load 3 blocks load two from moving in here.
...
def test_energy_and_restrictions_3_loads_c():
    ...
    expected = [
        ...
        {1: (2, 3)},
        {1: (3, 6)},  # load 1 takes shortest permitted path.
        {2: (5, 3)},
        {2: (3, 2)},
        {2: (2, 1)},  # load 2 moves to destination.
...
def test_energy_and_restrictions_2_load_high_detour_costs():
    ...
    for sed in [
        ...
        (3, 4, 1),
        (4, 6, 1),  # this could be shortest path for load 1, but
        # load 1 cannot travel over 4. If it could the path [2,3,4,6] would be 3 long.
        (2, 3, 1),
        (
            3,
...
def test_shuffle2():
    ...
    for s, e in edges:
        g.add_edge(s, e, 1, bidirectional=True)
    loads = {1: (1, 7), 2: (2, g.nodes()), 3: (8, g.nodes())}
...
def test_timeout():
    ...
    loads_for_jam_solver = {
        "L23": (41, [3, 4, 41, 44, 1, 2]),
        "L48": (42, ["pseudo_L48"]),
        "L33": (43, ["pseudo_L33"]),
        "L8": (44, [3, 4, 41, 44, 1, 2]),
        "L35": (45, ["pseudo_L35"]),
    ...
    moves = jam_solver(
        graph=subgraph_2,
        loads=loads_for_jam_solver,
        timeout=5000,
        synchronous_moves=False,
    )
