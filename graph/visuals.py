...
def visualise(func):
    def wrapper(*args, **kwargs):
        if not visuals_enabled:
            raise ImportError(
                "visualise is not available unless matplotlib is installed"
            )
...
def plot_2d(graph, nodes=True, edges=True):
    ...
    for node in graph.nodes():
        if not isinstance(node, tuple):
            raise ValueError(
                f"expected graph.nodes() to be tuple(x,y), but found {node}"
            )
        if not len(node) == 2:
            raise ValueError(
                f"expected tuples have 2 values, but found {node} (len={len(node)})"
            )
        x, y = node
        if not isinstance(x, (float, int)):
            raise ValueError(
                f"expected node in graph.nodes() to have (x,y) as float or int, but got {type(x)}"
            )
        if not isinstance(y, (float, int)):
            raise ValueError(
                f"expected node in graph.nodes() to have (x,y) as float or int, but got {type(y)}"
            )
