...
def doit(str_game):
    ...
    g.nodes = [Node(i, x=math.sin(i*delta_angle), y=math.cos(i*delta_angle)) for i in range(size)]
    g.edges = []
  ...
  def recurse(self, path):
    ...
    for edge in last_node.edges:
      ...
      if new_node in path:
        # This is a circle
        continue
      self.recurse(path + [new_node,])

  def all(self, nodes):
    for node in nodes:
      self.recurse(path=[node,])

def plot_2d(g):
    matplotlib.pyplot.figure()

    for edge in g.edges:
        matplotlib.pyplot.plot([edge.a.x, edge.b.x], [edge.a.y, edge.b.y], 'bo-', clip_on=False)

    for node in g.nodes:
      matplotlib.pyplot.annotate(node.i,
      (node.x, node.y), 
